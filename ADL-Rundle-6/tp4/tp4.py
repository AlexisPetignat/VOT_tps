...
def preprocess_patch(patch: np.ndarray):
    ...
    roi_input = cv2.resize(patch, (MODEL_WIDTH, MODEL_HEIGHT))
...
def compute_iou_matrix(
    active_tracks: list[Track], current_detections: list[BoundingBox]
):
    cost_matrix = np.ones((len(active_tracks), len(current_detections))) * 1e3
...
def compute_sim_matrix(
    active_tracks: list[Track], current_detections: list[BoundingBox], frame: np.ndarray
):
    preprocessed_pred = []
    preprocessed_det = []
    # Build preprocessed prediction patches
    for track in active_tracks:
        # Extract box
        new_center = track.predict_kalman()
        track_box = track.box
        track_box.move_center(new_center=new_center)

        # Crop the image
        top_left = track_box.top_left
        bot_right = track_box.bot_right
        patch = frame[
            int(top_left.y) : int(bot_right.y), int(top_left.x) : int(bot_right.x), :
        ]

        prep_patch = preprocess_patch(patch=patch)
        preprocessed_pred.append(prep_patch)

    # Build preprocessed detections
    for box in current_detections:
        # Crop the image
        top_left = box.top_left
        bot_right = box.bot_right
        patch = frame[
            int(top_left.y) : int(bot_right.y), int(top_left.x) : int(bot_right.x), :
        ]

        prep_patch = preprocess_patch(patch=patch)
        preprocessed_det.append(prep_patch)

    pred_feature = session.run(None, {"modelInput": np.array(preprocessed_pred)})[0]
    det_feature = session.run(None, {"modelInput": np.array(preprocessed_det)})[0]
    return 1 - cosine_similarity(pred_feature, det_feature)
    ...
    for frame_idx, frame_num in enumerate(frame_numbers):
        ...
        if frame_idx == 0:
            ...
            if len(active_tracks) > 0 and len(current_detections) > 0:
                ...
                iou_matrix = compute_iou_matrix(active_tracks, current_detections)
                cos_sim_matrix = compute_sim_matrix(
                    active_tracks, current_detections, frame
                )
                cost_matrix = ALPHA * iou_matrix + BETA * cos_sim_matrix
